...
def recommend():
   ...
   for i in range(number_of_recs):
      my_songs.append([str(top40.iloc[i,4]),str(top40.iloc[i,1]), "https://open.spotify.com/track/"+ str(top40.iloc[i,-6]).split("/")[-1],convert_from_ms(int(top40.iloc[i,-4])),str(top40.iloc[i,7])])
   return render_template('results.html',songs= my_songs)

   ##my_songs.append([str(top40.iloc[i,1]) + ' - '+ '"'+str(top40.iloc[i,4])+'"', "https://open.spotify.com/track/"+ str(top40.iloc[i,-6]).split("/")[-1],convert_from_ms(int(top40.iloc[i,-4]))])